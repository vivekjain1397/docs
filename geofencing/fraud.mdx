...
## [Bypassing checks for testing](#bypassing-checks-for-testing)
...
## [Allowed states and countries](#allowed-states-and-countries)
...
## [Verifying user locations](#verifying-user-locations)
...
### [Manual location checks](#manual-location-checks)
...
- `token`: A tamper-proof JSON Web Token (JWT) that you can send to your server to validate the signature, signed with the _JSON Web Token (JWT) Secret Key_ found under _Fraud_ on the [Settings page](https://radar.com/dashboard/settings#fraud-settings).
...
### [Automatic location checks with caching](#automatic-location-checks-with-caching)
...
To troubleshoot a failed geolocation check, go to the [Fraud page](https://radar.com/dashboard/fraud). Use the _Filters_ button to filter down to specific user IDs, IP addresses, or jurisdictions, and use the _date filters_ to filter down to specific dates or times.
...
## [User and device risk scores](#user-and-device-risk-scores)
...
To receive real-time email alerts when users reach high risk, and to receive a weekly report with high risk users, set a _Notification email_ under _Fraud_ on the [Settings page](https://radar.com/dashboard/settings#fraud-settings).
