...
## Verify integration data delivery[#](#verify-integration-data-delivery)
...
### Message all users nearby a store[#](#message-all-users-nearby-a-store)
...
```kotlin MyRadarReceiver.kt
class MyRadarReceiver : RadarReceiver() {
  override fun onLocationUpdated(context: Context, location: Location, user: RadarUser) {
    if (user.foreground && user.source == RadarLocationSource.FOREGROUND_LOCATION) {

      // Place detection
      user.place?.let { place ->
        CustomEvent.newBuilder("radar_fg_place_detected")
          .setInteractionType("location")
          .addProperty("radar_place_name", place.name)
          .addProperty("radar_place_categories", place.categories.joinToString(","))
          .addProperty("radar_place_chain_slug", place.chain?.slug ?: "")
          .build()
          .track()
      }

      // Geofence detection
      user.geofences?.takeIf { it.isNotEmpty() }?.let { geofences ->
        CustomEvent.newBuilder("radar_fg_geofences_detected")
          .setInteractionType("location")
          .addProperty("radar_geofence_descriptions", geofences.mapNotNull { it.description })
          .addProperty("radar_geofence_tags", geofences.mapNotNull { it.tag })
          .addProperty("radar_geofence_external_ids", geofences.mapNotNull { it.externalId })
          .build()
          .track()
      }
    }
...
### radar_geofence_entered[#](#radar_geofence_entered)
...
### radar_geofence_exited[#](#radar_geofence_exited)
...
### radar_dwelled_in_geofence[#](#radar_dwelled_in_geofence)
...
### radar_place_entered[#](#radar_place_entered)
...
### radar_place_exited[#](#radar_place_exited)
...
### radar_country_entered[#](#radar_country_entered)
...
### radar_country_exited[#](#radar_country_exited)
...
### radar_state_entered[#](#radar_state_entered)
...
### radar_state_exited[#](#radar_state_exited)
...
### radar_dma_entered[#](#radar_dma_entered)
...
### radar_dma_exited[#](#radar_dma_exited)
...
### radar_started_trip[#](#radar_started_trip)
...
### radar_updated_trip[#](#radar_updated_trip)
...
### radar_approaching_trip_destination[#](#radar_approaching_trip_destination)
...
### radar_arrived_at_trip_destination[#](#radar_arrived_at_trip_destination)
...
### radar_stopped_trip[#](#radar_stopped_trip)
...
### radar_beacon_entered[#](#radar_beacon_entered)
...
### radar_beacon_exited[#](#radar_beacon_exited)
